var y={consentOptions:[],cookieName:"tdecc",exceptionUrls:[],explanationAnchors:[],language:"nl",manageable:!0,style:"bar",renderSelector:"[data-tdecc-render]",version:"1.0"};var $={splash:{title:"Mogen we even je aandacht?",desc:"Om een persoonlijke ervaring te bieden en onze website te verbeteren, plaatsen wij cookies.",buttons:{manage:"Beheer cookies",accept:"Ik accepteer cookies"}},manage:{title:"Cookie instellingen",desc:`Wij laten je graag een goede website zien, afgestemd op jouw voorkeuren.
    ...
    </li>`;return`<ul class="tdecc__links">${t}</ul>`};var Y=(e,t)=>{let o="";for(let n=0;n<e.length;n+=1){let c=n+1;e[n]?.notCustomizable?o+=`<div class="tdecc__manage__option">
  ...
  </div>`;var V=()=>{let{content:e}=window.tdecc,{config:t}=window.tdecc;return!e||!e?(a("Content and config not found"),""):`
    ...
    </div>`:(a("Content not found"),"")};var X=()=>Date.now().toString(36)+Math.random().toString(36).substr(2),K=e=>{if(e.dataset.tdeccIdentifier)return;let t=X(),o=B();if(!o)return;let n=document.createElement("div");n.dataset.tdeccIdentifier=t,n.className="tdecc-notification",n.innerHTML=o,e.after(n),e.dataset.tdeccIdentifier=t},ee=e=>{let t=e.dataset.tdeccIdentifier;if(!t)return;let o=document.querySelector(`.tdecc-notification[data-tdecc-identifier="${t}"]`);o&&(o.outerHTML="")},G=()=>{let e=i();v(e).elements.forEach(t=>{if(!t.dataset.tdeccPermissions)return;let o=t.dataset.tdeccPermissions;if(o=o.replace(/\s+/g,""),!b(o.split(","))){t.hasAttribute("data-tdecc-show-notification")&&(K(t),delete t.dataset.tdeccShowNotification);return}ee(t);let n=document.createElement("textarea");n.innerHTML=t.innerHTML,F(t.parentElement,n.value),t.dataset.tdeccRendered="1"})};var T=()=>{L(i().cookieName)},U=()=>{let e=p(),t=i(),o=!0;if((!e.info||!e.info.v||e.info.v!==t.version)&&(o=!1),e.accepted||(o=!1),t.exceptionUrls.find(n=>n===window.location.pathname)&&(o=!0),o){window.tdecc.cookies=e,g(),k(e.accepted,e.info);return}T()};window.tdecc=window.tdecc||{};window.tdecc.initialized=!1;window.tdecc.accepted=[];window.tdecc.info={};window.tdecc.content={};var te=()=>({getAllPermissions(){return m()},checkPermission(e){return b(e)},show(){l(!0),u()},hide(){s()},update(){C(),g()},init(e){if(window.tdecc.initialized)return;let t=M(e),o=A(),n=f(t.cookieName)||!1;if(window.tdecc.config=t,window.tdecc.content=o[t.language],window.tdecc.config.consentOptions.unshift({key:"essential",title:window.tdecc.content.permissions.essential.title,desc:window.tdecc.content.permissions.essential.description,notCustomizable:!0}),window.tdecc.getAllPermissions=this.getAllPermissions,window.tdecc.checkPermission=this.checkPermission,window.tdecc.show=this.show,window.tdecc.hide=this.hide,window.tdecc.update=this.update,document.addEventListener("tdecc-changed",()=>{G()}),q(),n)try{U(),W()}catch{T(),l()}else l();Q(),C(),window.tdecc.initialized=!0}}),ft=te();export{ft as default};
